...
class analyze():
    ...
    def Ka_R(self, theta, phi) -> float:  # Rankine 一般狀況主動土壓係數
        # 轉成 rad
        alpha_D, theta_D, phi_D = self.alpha, theta, phi  # 單位:度
    ...
    def Kp_R(phi) -> float:  # Rankine 一般狀況被動土壓係數
        return tan(radians(45 + phi/2))**2

    def FS_fall(self) -> float:
        # 抗翻轉
        Ka = self.Ka_R(self, self.alpha, self.S2, self.Phi1)      # Rankine 主動土壓係數
        # print(f"Ka -> {Ka:.4f}")
        # Kp = 0      # Rankine 被動土壓係數
    ...
    def FS_slide(self):
        ...
        k2 = 2/3
        Kp = Kp_R(phi=Phi2)
        Pp = (Kp * r2 * D**2) / 2 + (2 * c2 * sqrt(Kp) * D)  # 計算 Rankine 被動土壓力
        # print(f"Pp -> {Pp:.4f}")
    ...
    def FS_carrying(self) -> float:
        # 承載值破壞之檢驗
        X_bar = (self.sum_MR - sum_MO) / sum_V
        e = B / 2 - X_bar       # 計算合力偏心值
        if e >= B/6:
